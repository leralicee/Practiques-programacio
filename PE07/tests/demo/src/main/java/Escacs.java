...
public class Escacs {
    ...
    public boolean esMovimentValidPerPeça(char peça, int filaOrigen, int colOrigen, int filaDesti, int colDesti,
    ...
    public int[] parsejarCoordenada(String coord) {
    ...
    // MÈTODES PER PROVES UNITÀRIES

    /**
     * Inicialitza un tauler buit (sense peces)
     * Útil per a proves unitàries
     */
    public void inicialitzarTaulerBuit() {
        for (int i = 0; i < MIDA_TAULER; i++) {
            for (int j = 0; j < MIDA_TAULER; j++) {
                tauler[i][j] = BUIT;
            }
    ...
    /**
     * Estableix el tauler amb un estat personalitzat
     * 
     * @param nouTauler El nou estat del tauler
     */
    public void setTauler(char[][] nouTauler) {
        this.tauler = nouTauler;
    }

    /**
     * Obté l'estat actual del tauler
     * 
    ...
    /**
     * Estableix de qui és el torn
     * 
     * @param torn true si és el torn de les blanques, false si és de les negres
     */
    public void setTornBlanques(boolean torn) {
